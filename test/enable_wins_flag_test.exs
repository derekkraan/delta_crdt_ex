defmodule DeltaCrdt.EnableWinsFlagTest do
  ...
  test "simultaneous enable and disable: enable wins" do
    ...
    assert true ==
             ...
             |> DeltaCrdt.JoinSemilattice.read()
  end
  ...
  test "with running CRDTs" do
    ...
    assert true == GenServer.call(crdt1, :read)

    send(crdt1, :ship_interval_or_state_to_all)
    Process.sleep(10)

    assert true == GenServer.call(crdt2, :read)

    assert true == GenServer.call(crdt3, :read)

    GenServer.cast(crdt2, {:operation, {DeltaCrdt.EnableWinsFlag, :disable, []}})
    send(crdt2, :ship_interval_or_state_to_all)
    Process.sleep(10)

    assert false == GenServer.call(crdt1, :read)

    assert false == GenServer.call(crdt2, :read)

    assert false == GenServer.call(crdt3, :read)
  end
end
